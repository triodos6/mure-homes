...
export async function generateMetadata({ params }) {
  ...
  const title = t('seo.productsTitle') || `${t('products.catalogTitle') || 'Nuestros Productos'} | MuraHomes`;
  const description = t('seo.productsDescription') || 'Explora nuestras colecciones de muebles de lujo excepcionales.';
...
export default async function LocalizedProductsPage({ params }) {
  const { locale } = await params;
  const messages = await getMessages(locale);
  const t = (k, p) => getMessage(messages, k, p);

  const LIMIT = 12;
  const [rawProducts, initialTotal] = await Promise.all([
  ...
  const initialProducts = rawProducts.map((p) => getLocalizedProduct(p, locale));
    ...
    <>
            ...
            <div className="flex flex-col justify-center">
              ...
              <div className="flex items-center gap-3 mb-4 sm:mb-6 animate-in fade-in slide-in-from-left-4 duration-700">
                ...
                <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.25em] sm:tracking-[0.3em] text-black/50">
                  {t('home.heroBadge') || '500+ Piezas Seleccionadas'}
                </span>
              </div>
              ...
              <p className="text-sm sm:text-base text-muted-foreground font-light leading-relaxed mb-6 sm:mb-8 max-w-lg animate-in fade-in slide-in-from-left-4 duration-700 delay-200 fill-mode-both">
                {t('products.catalogSubtitle') || 'Ocho mundos de muebles de lujo — desde salones hasta terrazas exteriores. Cada pieza seleccionada por su artesanía, materiales nobles y diseño atemporal.'}
              </p>
              ...
              <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8 flex-wrap text-xs text-muted-foreground font-medium animate-in fade-in duration-700 delay-300 fill-mode-both">
                <span className="inline-flex items-center gap-1.5 bg-white/80 backdrop-blur-sm px-3 py-1.5 rounded-full border border-black/5 shadow-sm">
                  <ShieldCheck size={14} className="text-amber-500" /> {t('product.warrantyBadge') || 'Garantía 2 Años'}
                </span>
                <span className="inline-flex items-center gap-1.5 bg-white/80 backdrop-blur-sm px-3 py-1.5 rounded-full border border-black/5 shadow-sm">
                  <Sparkles size={14} className="text-amber-500" /> {t('home.showroomCTA') || 'Asesoramiento Privado'}
                </span>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 animate-in fade-in slide-in-from-left-4 duration-700 delay-400 fill-mode-both">
                ...
                >
                  {t('navigation.viewAll') || 'Ver Todo'} <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
                </a>
                <a
                  href="#categories"
                ...
                >
                  {t('home.categoriesTitle') || 'Por Categoría'}
                </a>
              </div>
            </div>
          ...
          <div className="flex items-end justify-between mb-10">
            <div>
              <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-muted-foreground block mb-2">
                {t('home.categoriesTitle') || 'Explorar por'}
              </span>
              <h2 className="font-serif text-3xl font-semibold">
                {t('home.categoriesSubtitle') || 'Nuestras Categorías'}
              </h2>
            </div>
          </div>
      ...
      <section id="products" className="py-20 bg-[#f9f7f4]">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="mb-10">
            <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-muted-foreground block mb-2">
              {t('products.catalogSubtitle') || 'El Catálogo Completo'}
            </span>
            <h2 className="font-serif text-3xl font-semibold">
              {t('products.catalogTitle') || 'Todos los Productos'}
            </h2>
          </div>
          <ProductsGrid
      ...
      <section className="py-16 bg-black">
        <div className="max-w-7xl mx-auto px-6 lg:px-12 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-amber-400 mb-2">
              {t('home.showroomBannerTitle') || '¿Necesitas ayuda para elegir?'}
            </p>
            <h3 className="font-serif text-2xl font-medium text-white">
              {t('home.showroomBannerDesc') || 'Reserva una consulta de diseño privada'}
            </h3>
          </div>
          <Link
            href={locale === 'es' ? '/showroom' : `/${locale}/showroom`}
          ...
          >
            {t('home.showroomCTA') || 'Reservar Ahora'} <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
          </Link>
        </div>
      </section>
    </>
