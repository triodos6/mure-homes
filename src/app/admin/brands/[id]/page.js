...
export default function EditBrandPage() {
      ...
      try {
        ...
        SUPPORTED_LOCALES.forEach((loc) => {
          ...
          safeTranslations[loc] = {
            name: trans.name || (loc === 'es' ? brand.name : ''),
            slug: trans.slug || (loc === 'es' ? brand.slug : ''),
            description: trans.description || (loc === 'es' ? brand.description : ''),
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success('Brand partner removed from registry');
      router.push('/admin/brands');
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center gap-4 text-muted-foreground animate-pulse font-serif">
        ...
        <p className="tracking-widest uppercase text-xs">Loading Artisan House Data...</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-28">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="font-serif text-3xl font-semibold text-foreground tracking-tight mt-1">
              {formData.name || 'Edit Brand'}
            </h1>
            <p className="text-xs text-muted-foreground font-light">
              Edit brand identity, craft narrative, and localized metadata.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6">
            ...
            {formData.logo && (
              <div className="pt-2">
                <button
                  ...
                  className="w-full py-2 text-xs font-bold text-destructive hover:bg-destructive/10 rounded-lg transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                >
          ...
          <section className="rounded-2xl border border-border/80 bg-white p-6 space-y-4 shadow-xs">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed font-light">
              This brand partner is linked across all catalogue collections and luxury interior design categories.
            </p>
          </section>
      ...
      {isModalOpen && (
        <DeleteModal
          isOpen={isModalOpen}
          itemName={formData.name || 'this brand partner'}
          isDeleting={isDeleting}
          onClose={() => setIsModalOpen(false)}
          onConfirm={confirmDelete}
          title="Remove Brand Partner"
          description={`Are you sure you want to permanently remove "${formData.name}" from the brand registry? This action cannot be undone.`}
        />
      )}
    </div>
  );
