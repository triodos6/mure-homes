...
export default function NewProductPage() {
  ...
  // Initializing empty translations for all 22 supported locales
  ...
  const handleTranslationChange = (field, value) => {
    setFormData((prev) => {
      ...
      // If activeTab is 'es' (base), also sync base formData fields
  ...
  // Copy Spanish base content to the currently active language tab
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name && !formData.translations?.es?.name) {
      toast.error('Please enter a product name for the default language (Spanish).');
      return;
    }
    ...
    if (!formData.price) {
      toast.error('Please specify a base price.');
      return;
    }
    ...
    try {
      ...
      toast.success(`${formData.name || formData.translations.es.name} created successfully!`);
      router.push('/admin/products');
    } catch (err) {
      toast.error(err.message || 'Error registering product');
    } finally {
      setSaving(false);
    }
  };

  // Calculate translation progress counts
  ...
  // Filter locales for the search bar
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-28">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="font-serif text-3xl font-semibold text-foreground tracking-tight mt-1">
              {formData.name || 'New Design Piece'}
            </h1>
            <p className="text-xs text-muted-foreground font-light">
              Create and configure a new luxury catalog piece.
            </p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Left Column — Translation Manager */}
        <div className="lg:col-span-2 space-y-6">
          ...
          <section className="rounded-2xl border border-border/80 bg-white p-6 sm:p-8 space-y-6 shadow-xs">
            ...
            {/* Language Selector Bar with Search */}
            ...
            <div className="flex items-center justify-between bg-[#fcfcfc] p-4 rounded-xl border border-border/60">
              ...
              {/* Status Selector for Active Language */}
              <div className="flex items-center gap-2">
            ...
            {/* SEO Meta Fields & Live Google SERP Preview */}
          ...
          <section className="rounded-2xl border border-border/80 bg-white p-6 sm:p-8 space-y-6 shadow-xs">
            <div className="flex items-center justify-between border-b border-border/60 pb-4">
              <div>
                <h2 className="text-lg font-serif font-semibold text-foreground flex items-center gap-2">
                  <ImageIcon size={18} className="text-primary" /> Gallery Imagery
                </h2>
