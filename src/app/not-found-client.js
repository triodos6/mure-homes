...
export default function NotFoundClient() {
  const { t, getLocalizedHref, messages } = useI18n();
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <div>
              ...
              <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl font-semibold leading-[1.1] text-foreground mb-6 -mt-8 sm:-mt-12">
                {t('notFound.titlePrefix') || 'Página'} <span className="text-amber-500">{t('notFound.titleHighlight') || 'no encontrada'}</span>
              </h1>

              <p className="text-base sm:text-lg text-muted-foreground font-light leading-relaxed mb-8 max-w-lg">
                {t('notFound.description') || 'La dirección que has introducido no existe o el contenido ha sido reubicado. Te sugerimos explorar nuestro catálogo de piezas mediterráneas o regresar a la página principal.'}
              </p>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                ...
                >
                  <Home size={14} /> {t('notFound.goHome') || 'Volver al Inicio'}
                </Link>
                <Link
                ...
                >
                  <Compass size={14} /> {t('notFound.viewCatalog') || 'Catálogo Completo'}
                </Link>
              </div>
            </div>
            ...
            <div className="relative h-[340px] sm:h-[420px] rounded-2xl overflow-hidden shadow-2xl group bg-secondary/30 hidden sm:block">
              ...
              <div className="absolute bottom-6 left-6 right-6 text-white">
                ...
                <p className="font-serif text-2xl font-medium">
                  {t('notFound.imageCaption') || 'Elegancia mediterránea para tu hogar.'}
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="py-20 bg-white border-t border-border/50">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="flex items-center justify-between mb-10">
            <div>
              <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-muted-foreground block mb-2">
                {t('notFound.exploreCatalog') || 'Explora Nuestro Catálogo'}
              </span>
              <h2 className="font-serif text-3xl font-semibold">
                {t('notFound.featuredCategoriesPrefix') || 'Categorías'} <span className="text-amber-500">{t('notFound.featuredCategoriesHighlight') || 'Destacadas'}</span>
              </h2>
            </div>
            <Link
              href={getLocalizedHref('/products')}
            ...
            >
              {t('navigation.viewAll') || 'Ver Todas'} <ArrowRight size={12} className="group-hover:translate-x-1 transition-transform" />
            </Link>
          </div>
                ...
                >
                  ...
                  <div className="absolute bottom-4 left-4 right-4">
                    ...
                    <span className="inline-flex items-center gap-1 text-white/60 text-[10px] uppercase tracking-wider mt-1 group-hover:text-white transition-colors">
                      {t('common.viewDetails') || 'Explorar'} <ArrowRight size={10} />
                    </span>
                  </div>
                </Link>
              );
          ...
          <div className="mt-8 text-center sm:hidden">
            ...
            >
              {t('navigation.viewAll') || 'Ver Todo el Catálogo'} <ArrowRight size={12} />
            </Link>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div>
            <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-amber-400 block mb-2">
              {t('notFound.showroomTag') || '¿Buscas algo específico?'}
            </span>
            <h3 className="font-serif text-2xl font-medium">
              {t('notFound.showroomTitle') || 'Visita nuestro showroom en Gipuzkoa o solicita una consulta privada'}
            </h3>
          </div>
          <Link
          ...
          >
            {t('home.bookShowroom') || 'Reservar Cita'} <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
          </Link>
        </div>
      </section>
    </div>
